...
plot.dabest <- function(dabest.object,
  ...
  if (is.null(float.contrast)) {
    float.contrast <- TRUE
    if ( is.null(slopegraph) & isTRUE(boot.result$paired[1]) ) {
      slopegraph <- TRUE
    } else {
      slopegraph <- FALSE
    }
  } else {
    if ( isTRUE(float.contrast) & isTRUE(boot.result$paired[1]) ) {
      if (is.null(slopegraph)) slopegraph <- TRUE
    }
  }
