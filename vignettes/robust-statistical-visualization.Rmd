...
gardner.altman <- 
  dabest(my.data, x = Group, y = Value, 
         ...
         paired = FALSE) %>%
  plot(palette = "Dark2",
       theme = non.floating.theme,
       rawplot.type = 'swarmplot',
       rawplot.markersize = 8, 
       swarmplot.params = list(priority = "random"))
...
```{r gardner.altman.unpaired, warning=FALSE, message=FALSE, echo=FALSE, fig.width = 6, fig.height = 3}
...
```{r gardner.altman.paired, warning=FALSE, message=FALSE, echo=FALSE, fig.width = 6, fig.height = 3}
two.group.paired <- 
  my.data %>%
  dabest(Group, Value, 
         idx = c("Control", "Test"), 
...
plot(shared.control, theme = custom.theme, 
     tick.fontsize = 13, axes.title.fontsize = 15,
     color.column = Gender)
