...
bootstrap.demo <- function(
  ...
  resamples.plot <-
    ...
    theme(
      legend.title         =  element_blank(),
      plot.title           =  element_text(hjust = 0, family = "Rubik Medium"),
      # margin units are top, right, bottom, left.
      plot.margin          = unit(c(0, 5, 0, 5), "pt")) +
...
```{r bootstrap.mean_diff.gif, animation.hook='gifski', interval = 0.05, fig.height=9}


bootstrap.demo(sample1, sample2, show.resamples = 3, resamples = 5000, 
               nbins = 100)

# To create the GIF, You will need to install the packages `gifski`, 
...
While resampling distributions of the difference in means often have a normal distribution, it is not uncommon to encounter a skewed distibution. Thus, Efron developed the [bias-corrected and accelerated bootstrap](https://en.wikipedia.org/wiki/Bootstrapping_(statistics)#History) (BCa bootstrap) to account for the skew, and still obtain the central 95% of the distribution. [dabestr](using-dabestr.html) applies the BCa correction to the resampling bootstrap distributions of the effect size. 
